...
class Board
  ...
  def detect_winner
    WINNING_LINES.each do |line| #each line is array of [1,2,3] or [4,5,6]
      #@squares.select is select key value pairs that includes the key
      #remember @squares is the entire hash, so only select key value pairs that is in line at thta iteration
      if count_human_marker(@squares.select{|k,_| line.include?(k)}.values) == 3 #use values to return an array, otherwise will return hash
